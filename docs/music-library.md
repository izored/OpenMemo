...
- **A missing file is not a downloaded track.** A memo keeps its `file_path` after the file itself disappears from disk, which is what a wiped or restored files directory looks like. Playlist progress resolves every path on disk (`resolve_memo_path`) rather than trusting the column, and reports what it cannot find as `missing`. Those tracks count into the header's re-download control, and starting the pass clears the stale path so the track reads as remote again instead of pretending to be playable. Before this, a playlist with 76 of 79 files gone showed as fully downloaded and offered to fetch 2.
...
- **Upload.** Three modes (`POST /api/ingest/album`, OPNMMO-0052): **Album** groups a dropped batch by each file's embedded album tag into one or more `music_kind=album` collections, ordered by tag track-number; **Playlist** drops the whole batch into one `music_kind=playlist` collection; **Tracks** files each song straight into the library as a music memo (`audio_kind=music`), the original behaviour. For album/playlist, cover precedence is **dropped image wins**: include one image alongside the audio and it is the art for everything created, otherwise the cover is extracted from art embedded in the files (`backend/core/audio_meta.py`, mutagen; FLAC/MP3/M4A/Ogg). Title, artist, and album come from each file's tags, falling back to the filename. Uploaded tracks are local + playable immediately (no download step) and `playlist_born`, so they live inside their album/playlist like any pulled collection. No dependence on the lossless relay.
