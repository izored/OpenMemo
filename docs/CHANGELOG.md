...
- 🌈 **Hover overlay gradient now accent-tinted** — the light-theme `.om-min-hover` gradient was hardcoded cream (`#f5f2ec`). Replaced with `color-mix(in srgb, var(--accent) 22%, transparent)` so the veil subtly picks up the user's chosen accent colour while remaining light and readable.
