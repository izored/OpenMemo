...
- 🗂️ **Spaces give a big side-project its own walls (backend, Phase 1).** A Space is a separate area above collections: its own memos and collections, hidden from the main dashboard so the everyday library stays clean. Under the hood a Space is just a workspace in the same `openmemo.db` (no second database, everything stays queryable), and every library list now defaults to the main workspace so a Space can never leak into All Memos. Deleting a Space is the one unrecoverable action in openMemo, so the server refuses unless you type the Space's exact name back, and a one-click export zips every memo to Markdown first as a backup. Foundation laid out in ADR-020; the sidebar, the Spaces page, and the typed-confirm delete screen come next.
