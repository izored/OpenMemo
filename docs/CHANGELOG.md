...
- ✏️ **Card pen moved under the delete ✕** — the edit-thumbnail pen lived alone in the top-left; it now joins the action cluster top-right, directly below the ✕. Minimal note cards (column actions) and the active music card (play cluster owns that corner) keep it on the left.
