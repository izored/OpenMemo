...
- 🎯 **Play all button looked cramped** — the playlist hero's primary button inherits the global primary style, whose tight 6 px right padding exists for buttons that end in a keyboard chip. This one doesn't, so it gets a proper 16 px right padding of its own.
