...
- 📱 **Drawer no longer leaves a gap below the foot on iPhone.** The full-screen drawer set both `inset:0` and `height:100dvh`; a fixed element honours height over bottom, so when the browser's reported `100dvh` did not match the visible viewport (e.g. Comet's URL bar) the drawer ended short and left dead space under the foot, stretching the player oddly. It now lets top/bottom govern, filling the visible viewport exactly.
