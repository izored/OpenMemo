...
- 📐 **Mobile space tidied up.** The drawer no longer leaves a dead gap under the foot: the player and foot anchor to the bottom while the collections list scrolls in the middle, and the floating "+" hides while the drawer is open. The dashboard drops the wide desktop side padding for a tight edge so cards use the screen, and the Music page rails now bleed to the screen edge (the next card peeks) instead of stopping short.
