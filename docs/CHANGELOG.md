...
- 🛠️ **The memo page grows a tool rail.** AI Summary and "Make it local" moved off the top of the page into a side rail that hugs the content, its top edge lined up with the video so nothing floats above the player anymore. The rail is twice as wide now, with room to add more tools later. On a phone it steps aside entirely; a summary you have already generated still shows as a toggle above your notes (OPNMMO-0042).
- 🤖 **Summaries now read the description and the transcript, and fetch the transcript for you.** Asking for a summary of a video or audio memo that has no transcript yet kicks off the caption-first (Whisper-fallback) pull in the background, then writes the summary from the description plus the spoken words once it lands. No captions and no audio to pull? It falls back to the description alone instead of dead-ending. Works for pulled web pages and voice notes too, since they all share one summary source.
- ⏱️ **Timestamps in a summary are clickable.** A Timestamp-mode summary renders each `mm:ss` as a blue control that seeks the player to that moment: the embedded video reloads at that second and plays, a local video jumps in place. The same clickable timestamps work in the rail and in the under-content copy.
- 🔁 **A generated summary lives in two places.** Read it in the rail as it generates, and reach it again from an "AI Summary" toggle under the content, beside Video description and Transcript. Sub-tabs switch between Timestamp, Key Insights, and Essay once you have more than one, and a model that answers in headings or bullets renders as headings or bullets.
...
- 🧲 **Video description and Transcript are collapsed by default.** They are toggles now, so opening a long video memo no longer dumps the whole description and transcript down the page. Tap one to expand, tap it again to close.
- 🗂️ **My Notes is its own card.** The notes area gets a boxed, tinted background like the AI Summary panel, but neutral instead of the accent, so it reads as your space and stays set apart from the rest of the memo.
