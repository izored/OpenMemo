...
- 🍎 **openMemo runs as a real macOS app.** A double-click app for Apple-Silicon Macs, built from a new `desktop/` Electron shell. The whole UI lives in its own window — no browser, no Docker — while a bundled Python backend boots behind it on a fixed local port and serves the built UI from its own origin. Your data lives in `~/Library/Application Support/OpenMemo`, so the read-only `.app` never has to write next to its own code. Ollama stays yours: the app never ships, installs, or launches it — it just points at your host:port (default `http://localhost:11434`, editable from the menu, no restart of your day). Build it on a Mac with `npm run dist` in `desktop/`; full guide in [docs/MACOS.md](MACOS.md).
...
- 🧩 **The capture extension defaults to the app's port.** The Chrome extension now points at `:8099` out of the box — the port the macOS app and the dev server share — so clipping a page lands straight in the running app with no setup. Docker users still switch it to `:8091` from the extension options.
