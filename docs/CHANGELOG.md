...
- 🎚️ **Slider handles stop getting clipped at the ends.** Every Appearance slider (Blur, Background fade, Gutter, …) shares one track style that ran full width, so at 0 (or max) the round handle's outer half spilled past the panel and the scroll container sliced it flat. The track is now inset by the handle's radius, so the handle stays whole at both ends — fixed once on the shared `.om-ap-range`, so no future slider can hit it.
