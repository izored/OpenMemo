...
- ☁️ **Cloud-download button on every not-local track.** A track that is remote (saved without auto-download), queued, or failed now wears a cloud-download badge in the center of its tile. Tap it to pull that one track to your device. A failed track shows the badge in red (tap to retry); a track mid-download shows the cloud spinning, no tap. The old tiny corner chip is now just a one-word status label (`not local`, `downloading`, `failed`).
- ⤓ **Re-download button on the playlist page.** When any track in a playlist is not on your device, a Re-download button sits in the hero next to Play all. It pulls every remote, queued, or failed track in one go, so a half-downloaded album finishes without hunting tile by tile. It steps aside only while a download is actively running (one pass at a time), and names the count when tracks have failed.
