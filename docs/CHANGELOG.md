...
- 🧹 **Clean feeds, everywhere** — playlist tracks live inside their playlist, full stop. They no longer flood All Memos, the type tabs, or the Music page library: the library is just the songs you saved one by one, like a liked-songs shelf. Open a playlist to see its tracks; delete a playlist and they move back to the library. Server-side exclusion, now with regression tests (`backend/tests/test_playlist_feed_filter.py`).
