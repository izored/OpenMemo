...
- 📁 **File/doc minimal card surface** — `[data-card="minimal"] .om-card-doc` now uses `var(--bg-rail)` (matches sidebar tint, follows theme changes) with a visible `--border-2` stroke instead of disappearing into the dashboard background. File-icon SVG (`.om-file-svg`) reduced 60% → 42% width / 140 → 98px max.
