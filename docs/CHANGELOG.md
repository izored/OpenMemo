...
- ✨ **New Memo panel closes without visual glitches.** Two split-second artifacts on close are gone: the text/content stretch (Framer Motion's layout projection bled through the panel's `overflow: visible` during animation) and the horizontal scrollbar flash that made the bottom bar jump (FM's default center scale-origin temporarily extended the island past the viewport's right edge). The island now uses `originX/Y: bottom-right` so it only ever grows left and up, and `overflow: hidden` is enforced for the duration of the layout animation via `onLayoutAnimationStart/Complete` so no content can bleed through.
