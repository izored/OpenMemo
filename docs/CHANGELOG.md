...
- 🔃 **New memos still sank to the bottom** — the previous fix only updated the changelog; the SQL in `list_memos` still ordered by `sort_order DESC, created_at DESC`, so any memo with a manually-set positive `sort_order` floated above freshly created ones. New approach: `GET /api/memos` accepts a `sort` query param (`recent` (default) / `oldest` / `title` / `custom`); only `custom` consults `sort_order`. The Dashboard reads/writes the chosen mode from `appStore` (persisted to localStorage as `openmemo_settings.sortMode`) and now ships a Sort dropdown in the filter rail. Drag-to-reorder auto-switches the user into `custom` mode so the manual order they just set is actually what they see after the next fetch.
