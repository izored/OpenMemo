...
**Date:** 2026-06-04 · **Status:** In progress (plan: [`.claude/plans/mobile-responsive-adr009.md`](../.claude/plans/mobile-responsive-adr009.md)) · **Relates to:** ADR-006 (sidebar is a fixed three-zone column), ADR-005 (the sidebar hosts the now-playing player), ADR-008 (keep work off the request/render hot path), ADR-001 (define shared things once, never scatter)

> **Full implementation plan + live status live in [`.claude/plans/mobile-responsive-adr009.md`](../.claude/plans/mobile-responsive-adr009.md).** This ADR is the high-level record; the plan file has the per-page table, file:line map, build-order checklist, blind-spots, and the commit log. **If you change any frontend layout, read the "Rules for every frontend change" section of that plan so the change stays responsive.**

### Context

OpenMemo was built desktop-first; no page is designed for a small screen. The shell (`.om-app`) is a fixed-rail + flexible-`.om-main` flexbox that never collapses, and the only `@media` rules are scattered one-off patches (Settings 1100px, Ask 1000px, detail 720px) — magic numbers duplicated across CSS and JS. On a phone every page breaks its own way: the rail eats the screen, the feed never drops below 2 columns, the sidebar player clips (the "drop to 90% zoom" workaround), and modals/FAB ignore the notch. Two constraints shape the fix: (1) the scattered breakpoints must be unified, not added to; (2) the desktop designs are still changing (esp. MemoDetail), so mobile must ride the *same* components, never a hand-copied snapshot.

### Decision

**Make every existing page responsive in place: one codebase, no separate mobile version, one shared set of breakpoints.** Same components on every screen, re-laid-out by width — so when a desktop design changes later, its mobile version changes with it for free. Works on iOS Safari + Android Chrome; not a second app to keep in sync.

One breakpoint scale, defined once and read by both CSS tokens and a single-`matchMedia` `useBreakpoint()` hook:
...
Eight decisions (detail + file map in the plan file): (1) one breakpoint set; (2) sidebar → off-canvas drawer below `lg` over a tap-to-dismiss scrim, opened by a hamburger in a slim mobile top bar; (3) logo → home everywhere, collapse moves to the chevron only; (4) dashboard feed → 1 column on phones; (5) sidebar player → fluid (`min-width:0` + `clamp` + wrap), never clips, on every screen; (6) every other page reflows to a single readable column, with "main + side panel" pages sharing one two-pane helper (beside → slide-over → sheet); (7) modals/panels/FAB go mobile-native (bottom sheets, `viewport-fit=cover`, `env(safe-area-inset-*)`); (8) touch first-class (Lenis off on touch, ≥44×44px targets).

### Alternatives considered

| Option | Why rejected |
|--------|--------------|
| **Separate mobile version (second component set / route tree)** | Doubles work and drifts — every desktop change redone twice. The goal is the opposite: change a page once, both sizes follow. |
| **Hand-tune each page's mobile CSS against current markup** | Couples mobile to layouts still changing (esp. MemoDetail); breaks at the next redesign. Reflow by role through a shared helper instead. |
| **Narrower static rail on phones** | A fixed rail at any width still steals scarce phone space. Off-canvas frees the full width. |
| **Full-bleed `100vw` drawer** | No visible dismiss target; reads as a page change. `min(86vw,320px)` over a scrim keeps "tap to close". |
| **Phone-only media query for the player crop** | The clip also happens under desktop zoom; fix the sizing (`min-width:0` + `clamp` + wrap) so both are covered. |
| **New `max-width` literals per component** | Exactly the drift this ADR removes. One token scale, read by CSS and the hook. |

### Consequences

- **New shared pieces:** breakpoint token set (CSS) + `useBreakpoint()`/`useIsMobile()` hook (JS) as the one source of truth; a mobile top bar + drawer scrim; a two-pane helper for "main + side panel" pages; `mobileDrawerOpen` state reusing the store's already-present-but-unused `sidebarOpen`/`toggleSidebar`.
- **Desktop redesigns carry to mobile automatically** for anything built from the shared helper + hook — the explicit safety net for the MemoDetail rework and anything after.
- **Player crop fixed at the root**, so the 90%-zoom workaround is gone on every screen.
- **Preserves ADR-005/006** (player internals, three-zone sidebar) and **ADR-008** (one `matchMedia` subscription, drawer animates with `transform`, no resize thrash).
- **Owner override (2026-06-13):** the mobile drawer is **full-screen (100vw)**, not the `min(86vw,320px)` scrim originally chosen above — the owner wants the whole screen on a phone, with a prominent close button + top-bar logo→home for dismissal. The big now-playing player is also forced as the **mobile default** regardless of the `playerSize` appearance pref. Do not "correct" these back to the original table.
- **Status: in progress.** Build order, checklist, and what has actually landed are tracked in the plan file's commit log — not duplicated here.
